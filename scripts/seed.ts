...
function generateProductivityData(insertedEmployees: any[]): any[] {
  ...
  for (let date = new Date(startDate); date <= new Date(); date.setDate(date.getDate() + 1)) {
    ...
    insertedEmployees.forEach((employee, _index) => {
...
async function seedDatabase() {
  ...
  try {
    ...
    console.log('- Visit the dashboard at: http://localhost:4000')
    console.log('- View the API at: http://localhost:4001/cubejs-api/v1/meta')
